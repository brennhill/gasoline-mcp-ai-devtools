...
let chatEscapeHandler: ((e: KeyboardEvent) => void) | null = null
 ...
 */
export function toggleChatWidget(clientName?: string): void {
  if (clientName) currentClientName = clientName
...
function showChatWidget(): void {
  ...
  chatEscapeHandler = (e: KeyboardEvent) => {
    if (e.key === 'Escape') {
      e.stopImmediatePropagation()
      removeChatWidget()
    }
  ...
  const focusable: HTMLElement[] = [input, pinBtn, closeBtn]
  widget.addEventListener('keydown', (e: KeyboardEvent) => {
    if (e.key !== 'Tab') return
...
function removeChatWidget(): void {
...
function sendChatMessage(): void {
  ...
  chrome.runtime.sendMessage(
    ...
    (response: { success: boolean; status?: string; error?: string } | undefined) => {
