...
export async function handleLogMessage(payload: LogEntry, sender: ChromeMessageSender, tabId?: number): Promise<void> {
  if (!communication.shouldCaptureLog(payload.level, currentLogLevel, (payload as { type?: string }).type)) {
    debugLog(DebugCategory.CAPTURE, `Log filtered out: level=${payload.level}, type=${(payload as { type?: string }).type}`);
    return;
  }
...
export async function checkConnectionAndUpdate(): Promise<void> {
  ...
  try {
    ...
    if (health.connected) {
      ...
      polling.startQueryPolling(
        () => pollPendingQueriesWrapper(),
        debugLog
      );
      polling.startSettingsHeartbeat(
        () => postSettingsWrapper(),
        debugLog
      );
      polling.startWaterfallPosting(
        () => postNetworkWaterfall(),
        debugLog
      );
      polling.startExtensionLogsPosting(
