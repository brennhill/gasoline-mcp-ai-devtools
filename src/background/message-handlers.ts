...
export interface MessageHandlerDependencies {
  ...
  addToWsBatcher: (event: unknown) => void;
  addToEnhancedActionBatcher: (action: unknown) => void;
  addToNetworkBodyBatcher: (body: unknown) => void;
  addToPerfBatcher: (snapshot: unknown) => void;
...
export function installMessageListener(deps: MessageHandlerDependencies): void {
  ...
  chrome.runtime.onMessage.addListener(
    (message: BackgroundMessage, sender: any, sendResponse: SendResponse): boolean => {
      // SECURITY: Validate sender before processing any message
      if (!isValidMessageSender(sender)) {
        deps.debugLog(
          'error',
          'Rejected message from untrusted sender',
          { senderId: (sender as any).id, senderUrl: sender.url }
        );
        return false;
      }
...
async function handleClearLogsAsync(
  sendResponse: SendResponse,
  deps: MessageHandlerDependencies
): Promise<void> {
  try {
