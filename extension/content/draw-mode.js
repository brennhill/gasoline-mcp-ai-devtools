...
function buildElementDetail(el) {
    ...
    const styleProps = [
        ...
        'border-radius', 'border', 'margin', 'display',
...
function runA11yChecks(el, computed) {
    ...
    if (interactiveTags.includes(tag)) {
        const hasLabel = getAttribute('aria-label') ||
    ...
    try {
        if ((tag === 'input' || tag === 'select' || tag === 'textarea') && !getAttribute('aria-label')) {
            const id = el.id;
            const hasLabelFor = id && typeof document !== 'undefined' &&
    ...
    saveTimeout = setTimeout(() => {
        if (typeof chrome !== 'undefined' && chrome.storage?.session) {
            try {
                ...
                const toStore = annotations.length > MAX_PERSISTED_ANNOTATIONS
                    ? annotations.slice(-MAX_PERSISTED_ANNOTATIONS)
                    : annotations;
...
/**
 * Called when user presses ESC (no active text input) or from popup toggle.
 * Captures screenshot WHILE overlay is still visible, then deactivates and sends results.
 */
function deactivateAndSendResults() {
    if (!active) return;
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime) {
        try {
            chrome.runtime.sendMessage(
                { type: 'GASOLINE_CAPTURE_SCREENSHOT' },
                (screenshotResponse) => {
