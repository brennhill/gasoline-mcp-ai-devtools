...
async function loadFlags() {
    const stored = (await getLocal(ANALYTICS_STORAGE.DAILY_FLAGS));
    if (stored) {
 ...
 */
export function trackCommandUsage(commandType) {
 ...
 */
export function trackAiConnected() {
 ...
 */
export async function handleAnalyticsAlarm() {
    await persistFlags();
    await sendPing();
}
