...
export class ConnectionStateMachine {
    ...
    static transitions = {
        ...
        // Health check events
        HEALTH_OK: (n) => { n.lastHealthCheck = Date.now(); if (n.server !== 'up')
            n.server = 'up'; if (n.extension === 'disconnected')
            n.extension = 'connected'; },
        HEALTH_FAIL: (n) => { n.lastHealthCheck = Date.now(); if (n.extension !== 'disconnected') {
            n.extension = 'disconnected';
            n.polling = 'stopped';
        } },
        // Polling events
        POLLING_STARTED: (n) => { n.polling = 'running'; if (n.extension === 'connected')
            n.extension = 'active'; },
        POLLING_STOPPED: (n) => { n.polling = 'stopped'; if (n.extension === 'active')
            n.extension = 'connected'; },
        ...
        // Command events
        COMMAND_QUEUED: (n) => { if (n.commands === 'none')
            n.commands = 'queued'; },
