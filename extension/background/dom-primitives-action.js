 ...
 */
export function domPrimitiveAction(action, selector, options) {
    // --- Shadow DOM traversal ---
    function getShadowRoot(el) { return el.shadowRoot ?? null; }
    ...
    function resolveDeepCombinator(sel, root = document) {
    ...
    function isKaboomOwnedElement(element) {
        ...
        while (node) {
            ...
            if (typeof cn === 'string' && cn.includes('kaboom-'))
                return true;
    ...
    function getElementHandleStore() {
        ...
        const created = { byElement: new WeakMap(), byID: new Map(), selectorByID: new Map(), nextID: 1 };
    ...
    function getOrCreateElementID(el) {
        ...
        store.byElement.set(el, eid);
        store.byID.set(eid, el);
        return eid;
    }
    function resolveElementByID(rawEID) {
        ...
        if (storedSel) {
            ...
            if (re && re.isConnected !== false) {
                store.byElement.set(re, eid);
                store.byID.set(eid, re);
                return re;
            }
        }
        if (node)
            store.byID.delete(eid);
    ...
    function resolveByTextAll(searchText, scope = document) {
        ...
        function walkScope(root) {
            ...
            const ch = 'children' in root ? root.children : root.body?.children || root.documentElement?.children;
            if (ch) {
                for (let i = 0; i < ch.length; i++) {
                    const c = ch[i];
                    const s = getShadowRoot(c);
                    if (s)
                        walkScope(s);
                }
            }
    ...
    function resolveByLabelAll(labelText, scope = document) {
        const labels = querySelectorAllDeep('label', scope), results = [], seen = new Set();
    ...
    function resolveByAriaLabelAll(al, scope = document) {
        const results = [], seen = new Set();
        for (const el of querySelectorAllDeep(`[aria-label="${CSS.escape(al)}"]`, scope)) {
            if (!seen.has(el)) {
                seen.add(el);
                results.push(el);
            }
        ...
        for (const el of querySelectorAllDeep('[aria-label]', scope)) {
            ...
            if (lb.startsWith(al) && !seen.has(el)) {
                seen.add(el);
                results.push(el);
            }
        }
    ...
    function resolveElements(sel, scope = document) {
        if (!sel)
            return [];
        if (sel.includes(' >>> ')) {
            const d = resolveDeepCombinator(sel, scope);
            return d ? [d] : [];
        }
        const p = parseNthMatchSelector(sel);
        if (p) {
            const m = resolveElements(p.base, scope);
            const t = m[p.n - 1];
            return t ? [t] : [];
        }
        if (sel.startsWith('text='))
            return resolveByTextAll(sel.slice(5), scope);
        if (sel.startsWith('role='))
            return querySelectorAllDeep(`[role="${CSS.escape(sel.slice(5))}"]`, scope);
        if (sel.startsWith('placeholder='))
            return querySelectorAllDeep(`[placeholder="${CSS.escape(sel.slice(12))}"]`, scope);
        if (sel.startsWith('label='))
            return resolveByLabelAll(sel.slice(6), scope);
        if (sel.startsWith('aria-label='))
            return resolveByAriaLabelAll(sel.slice(11), scope);
        try {
    ...
    function resolveElement(sel, scope = document) {
        ...
        if (sel.startsWith('text='))
            return resolveByText(sel.slice(5), scope);
        if (sel.startsWith('role='))
            return firstVisible(querySelectorAllDeep(`[role="${CSS.escape(sel.slice(5))}"]`, scope));
        if (sel.startsWith('placeholder='))
            return firstVisible(querySelectorAllDeep(`[placeholder="${CSS.escape(sel.slice(12))}"]`, scope));
        if (sel.startsWith('label='))
            return resolveByLabel(sel.slice(6), scope);
        if (sel.startsWith('aria-label='))
            return resolveByAriaLabel(sel.slice(11), scope);
    ...
    function buildUniqueSelector(el, htmlEl, fb) {
        if (el.id)
        ...
        if (al)
            return `[aria-label="${CSS.escape(al)}"]`;
        const ph = el.getAttribute('placeholder');
        if (ph)
            return `[placeholder="${CSS.escape(ph)}"]`;
    ...
    function collectDialogs() {
        const d = [];
        for (const s of ['[role="dialog"]', '[aria-modal="true"]', 'dialog[open]'])
            d.push(...querySelectorAllDeep(s));
    ...
    function findTopmostOverlay() {
        ...
        const cands = [];
        for (const s of sels)
            cands.push(...querySelectorAllDeep(s));
    ...
    function describeOverlay(el) {
        ...
        const os = el.id ? `#${el.id}` : role ? `${tag}[role="${role}"]` : (() => { const cn = el.className; return typeof cn === 'string' && cn.trim() ? `${tag}.${cn.trim().split(/\s+/)[0]}` : tag; })();
    ...
    function detectOverlayWarning(targetEl) {
        ...
        if (!o)
            return {};
        if (typeof o.contains === 'function' && o.contains(targetEl))
            return {};
    ...
    function blockedByOverlayError(target) {
        ...
        if (!top)
            return null;
        if (typeof top.contains === 'function' && top.contains(target))
            return null;
    ...
    function summarizeCandidates(matches) {
        return matches.slice(0, 8).map((c) => {
            const h = c, fb = c.tagName.toLowerCase();
    ...
    function rankAmbiguousCandidates(candidates, act, selectorText) {
        ...
        const scored = candidates.map((el) => {
            ...
            if (selectorLabel) {
                const lb = extractElementLabel(el).trim();
                if (lb === selectorLabel)
                    score += 80;
                else if (lb.startsWith(selectorLabel) && lb.length <= selectorLabel.length + 5)
                    score += 60;
            }
            if (tag === 'button' || role === 'button') {
                const h = el, cls = (typeof h.className === 'string' ? h.className : '').toLowerCase(), tp = el.getAttribute('type') || '';
                if (tp === 'submit')
                    score += 60;
                else if (/\bprimary\b|\bbtn-primary\b|\bcta\b/.test(cls))
                    score += 60;
                else {
                    const st = typeof getComputedStyle === 'function' ? getComputedStyle(h) : null;
                    if (st) {
    ...
    function resolveActionTarget() {
        ...
        if (reqScope && !scopeRoot)
            return { error: domError('scope_not_found', `No scope element matches selector: ${reqScope}`) };
        ...
        if (reqEID) {
            ...
            if (!resolved)
                return { error: domError('stale_element_id', `Element handle is stale or unknown: ${reqEID}. Call list_interactive again.`) };
            if (activeScope !== document && typeof activeScope.contains === 'function' && !activeScope.contains(resolved))
                return { error: domError('element_id_scope_mismatch', `Element handle does not belong to scope: ${reqScope || '<none>'}`) };
            if (scopeRect && !intersectsScopeRect(resolved))
                return { error: domError('element_id_scope_mismatch', `Element handle does not intersect scope_rect.`) };
        ...
        if (nthParam !== undefined && nthParam !== null) {
            ...
            if (ri < 0 || ri >= cands.length)
                return { error: domError('nth_out_of_range', `nth=${nth} is out of range — selector matched ${cands.length} element(s). Use nth 0..${cands.length - 1} or -1..-${cands.length}.`) };
        ...
        const vm = (() => { if (rsm.length === 0)
            return rsm; const v = rsm.filter(isActionableVisible); return v.length > 0 ? v : rsm; })();
        if (vm.length > 1) {
            const ranking = rankAmbiguousCandidates(vm, action, selector);
            const tc = ranking.ranked.slice(0, 3).map((e) => ({ element_id: getOrCreateElementID(e.element), tag: e.element.tagName.toLowerCase(), text_preview: (e.element.textContent || '').trim().slice(0, 60) || undefined, score: e.score }));
            if (ranking.winner)
                return { element: ranking.winner, match_count: 1, match_strategy: 'ranked_resolution', ranked_candidates: tc };
    ...
    function matchedTarget(node) {
        const h = node, tp = (h.textContent || '').trim().slice(0, 80);
        ...
        return new Promise((resolve) => {
            ...
            function finish() {
                ...
                if (options.observe_mutations) {
                    const max = 50, entries = [];
                    for (const m of muts) {
                        if (entries.length >= max)
                            break;
                        if (m.type === 'childList') {
                            for (let i = 0; i < m.addedNodes.length && entries.length < max; i++) {
                                const n = m.addedNodes[i];
                                if (n && n.nodeType === 1) {
                                    const e = n;
                                    entries.push({ type: 'added', tag: e.tagName?.toLowerCase(), id: e.id || undefined, class: e.className?.toString()?.slice(0, 80) || undefined, text_preview: e.textContent?.slice(0, 100) || undefined });
                                }
                            }
                            for (let i = 0; i < m.removedNodes.length && entries.length < max; i++) {
                                const n = m.removedNodes[i];
                                if (n && n.nodeType === 1) {
                                    const e = n;
                                    entries.push({ type: 'removed', tag: e.tagName?.toLowerCase(), id: e.id || undefined, class: e.className?.toString()?.slice(0, 80) || undefined, text_preview: e.textContent?.slice(0, 100) || undefined });
                                }
                            }
                        }
                        else if (m.type === 'attributes' && m.target.nodeType === 1) {
                            const e = m.target;
                            entries.push({ type: 'attribute', tag: e.tagName?.toLowerCase(), id: e.id || undefined, attribute: m.attributeName || undefined, old_value: m.oldValue?.slice(0, 100) || undefined, new_value: e.getAttribute(m.attributeName || '')?.slice(0, 100) || undefined });
                        }
                    }
    ...
    function detectRichEditor(node) {
        ...
        for (const { selector: s, type: t } of [{ selector: '.ql-editor', type: 'quill' }, { selector: '.ProseMirror', type: 'prosemirror' }, { selector: '[data-contents="true"]', type: 'draftjs' }, { selector: '[data-editor]', type: 'draftjs' }, { selector: '.mce-content-body', type: 'tinymce' }, { selector: '#tinymce', type: 'tinymce' }, { selector: '.ck-editor__editable', type: 'ckeditor' }]) {
            if (typeof h.matches === 'function' && h.matches(s))
                return { type: t, target: h };
            if (typeof h.closest === 'function') {
                const a = h.closest(s);
                if (a instanceof HTMLElement)
                    return { type: t, target: a };
            }
        }
        return null;
    }
    function insertViaRichEditor(_t, target, text, clear) {
    ...
    function insertViaKeyboardSim(node, text) { for (const ch of text)
        dispatchKeySequence(node, ch, true); }
    // --- Scroll helpers ---
    function findScrollableContainer(el2) {
        ...
        while (cur && cur !== document.documentElement) {
            if (cur instanceof HTMLElement && cur.scrollHeight > cur.clientHeight + 10) {
                ...
                if (s && (s.overflow === 'auto' || s.overflow === 'scroll' || s.overflowY === 'auto' || s.overflowY === 'scroll'))
                    return cur;
            }
    ...
    function findInteractiveAncestor(el2) {
        ...
        if (new Set(['a', 'button', 'input', 'select', 'textarea']).has(tag) || new Set(['button', 'link', 'menuitem', 'tab', 'option', 'switch']).has(role))
            return null;
        if (typeof el2.closest === 'function') {
            const a = el2.closest('a, button, [role="button"], [role="link"], [role="menuitem"], [role="tab"], input, select, textarea');
            if (a && a !== el2)
                return a;
        }
        return null;
    ...
    const handlers = {
        click: () => withMutationTracking(() => {
            if (!(el instanceof HTMLElement))
                return domError('not_interactive', `Element is not an HTMLElement: ${el.tagName}`);
            const ia = findInteractiveAncestor(el), ct = (ia instanceof HTMLElement ? ia : el);
            const oe = blockedByOverlayError(el);
            if (oe)
                return oe;
            if (options.new_tab) {
                const ln = (() => { const t = ct.tagName.toLowerCase(); if (t === 'a')
                    return ct; return typeof ct.closest === 'function' ? ct.closest('a[href]') : null; })();
                const href = ln ? (ln.getAttribute('href') || ln.href || '') : '';
                if (!href)
                ...
                if (!opened && ln instanceof Element) {
                    const pt = ln.getAttribute('target');
                    ln.setAttribute('target', '_blank');
                    ln.click();
                    if (pt == null)
                        ln.removeAttribute('target');
                    else
                        ln.setAttribute('target', pt);
                }
                return mutatingSuccess(ct, { value: href, reason: 'opened_new_tab' });
            }
            const didScroll = autoScrollIfNeeded(ct);
            ct.click();
        ...
        select: () => withMutationTracking(() => {
            ...
            if (!(el instanceof HTMLSelectElement))
                return domError('not_select', `Element is not a <select>: ${el.tagName}`);
            const ns = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value')?.set;
            if (ns)
                ns.call(el, options.value || '');
            else
        ...
        check: () => withMutationTracking(() => {
            ...
            if (!(el instanceof HTMLInputElement) || (el.type !== 'checkbox' && el.type !== 'radio'))
                return domError('not_checkable', `Element is not a checkbox or radio: ${el.tagName} type=${el.type || 'N/A'}`);
        ...
        focus: () => {
            ...
            if (oe)
                return oe;
            if (!(el instanceof HTMLElement))
                return domError('not_focusable', `Element is not an HTMLElement: ${el.tagName}`);
            el.focus();
        ...
        hover: () => withMutationTracking(() => {
            ...
            el.dispatchEvent(new MouseEvent('mouseover', ei));
            el.dispatchEvent(new MouseEvent('mousemove', ei));
            return mutatingSuccess(el);
        }),
        paste: () => withMutationTracking(() => {
            ...
            let strategy;
            const editor = detectRichEditor(el);
            if (editor && el.isContentEditable) {
        ...
        key_press: () => withMutationTracking(() => {
            ...
            const km = { Enter: { key: 'Enter', code: 'Enter', keyCode: 13 }, Tab: { key: 'Tab', code: 'Tab', keyCode: 9 }, Escape: { key: 'Escape', code: 'Escape', keyCode: 27 }, Backspace: { key: 'Backspace', code: 'Backspace', keyCode: 8 }, ArrowDown: { key: 'ArrowDown', code: 'ArrowDown', keyCode: 40 }, ArrowUp: { key: 'ArrowUp', code: 'ArrowUp', keyCode: 38 }, Space: { key: ' ', code: 'Space', keyCode: 32 } };
        ...
        scroll_to: () => {
            function scrollToY(c, top) { if (typeof c.scrollTo === 'function')
                c.scrollTo({ top, behavior: 'smooth' });
            else
                c.scrollTop = top; }
            function scrollByY(c, dy) { if (typeof c.scrollBy === 'function')
                c.scrollBy({ top: dy, behavior: 'smooth' });
            else
                c.scrollTop = (Number(c.scrollTop) || 0) + dy; }
            ...
            const dc = (() => { if (isContainer)
                return el; const a = findScrollableContainer(el); if (a)
                return a; if (document.scrollingElement instanceof HTMLElement)
                return document.scrollingElement; return document.documentElement; })();
            if (dir && dc) {
                switch (dir) {
                    case 'top':
                        scrollToY(dc, 0);
                        return mutatingSuccess(el, { reason: 'scrolled_container_top' });
                    case 'bottom':
                        scrollToY(dc, dc.scrollHeight);
                        return mutatingSuccess(el, { reason: 'scrolled_container_bottom' });
                    case 'up':
                        scrollByY(dc, -dc.clientHeight * 0.8);
                        return mutatingSuccess(el, { reason: 'scrolled_container_up' });
                    case 'down':
                        scrollByY(dc, dc.clientHeight * 0.8);
                        return mutatingSuccess(el, { reason: 'scrolled_container_down' });
                }
            }
            if (tag === 'body' || tag === 'html') {
                const sc = findScrollableContainer(document.body);
                if (sc) {
                    sc.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    return mutatingSuccess(el, { reason: 'scrolled_nested_container' });
                }
            }
            const pc = findScrollableContainer(el);
            if (pc && pc !== document.documentElement) {
                el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                return mutatingSuccess(el, { reason: 'scrolled_within_container' });
            }
            el.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return mutatingSuccess(el);
        },
    ...
    if (!handler)
        return domError('unknown_action', `Unknown action: ${action}`);
    return handler();
}
//# sourceMappingURL=dom-primitives-action.js.map