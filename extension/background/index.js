...
export const logBatcherWithCB = communication.createBatcherWithCircuitBreaker(
...
export const wsBatcherWithCB = communication.createBatcherWithCircuitBreaker(
...
export const enhancedActionBatcherWithCB = communication.createBatcherWithCircuitBreaker(
...
export const networkBodyBatcherWithCB = communication.createBatcherWithCircuitBreaker(
...
export const perfBatcherWithCB = communication.createBatcherWithCircuitBreaker(
...
export async function handleLogMessage(payload, sender, tabId) {
  if (!communication.shouldCaptureLog(payload.level, currentLogLevel, payload.type)) {
    debugLog(
      ...
      `Log filtered out: level=${payload.level}, type=${payload.type}` // nosemgrep: missing-template-string-indicator
    )
    return
  }
