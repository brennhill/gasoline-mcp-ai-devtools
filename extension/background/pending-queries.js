    ...
    try {
        ...
        if (query.type === 'dom') {
            try {
                ...
                if (query.correlation_id) {
                    sendQueryAsyncResult(syncClient, query.id, query.correlation_id, 'complete', null, err.message || 'Failed to execute DOM query');
                }
                else {
                    sendQueryResult({
        ...
        if (query.type === 'a11y') {
            try {
                ...
                if (query.correlation_id) {
                    sendQueryAsyncResult(syncClient, query.id, query.correlation_id, 'complete', null, err.message || 'Failed to execute accessibility audit');
                }
                else {
                    sendQueryResult({
