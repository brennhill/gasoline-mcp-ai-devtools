...
export function domPrimitiveRead(action, selector, options) {
    ...
    function querySelectorAllDeep(sel, root = document, results = [], depth = 0) {
        if (depth > 10)
            return results;
        for (const match of Array.from(root.querySelectorAll(sel))) {
    ...
    function resolveScopeRoot(rawScope) {
        ...
        try {
            ...
            return m.length === 0 ? null : firstVisible(m) || m[0] || null;
        }
        catch {
            return null;
        }
    ...
    const handlers = {
        ...
        get_value: () => {
            if (!('value' in el))
                return domError('no_value_property', `Element has no value property: ${el.tagName}`);
