...
export async function requestWorkspaceAudit(pageUrl) {
    try {
        await openWorkspace();
    }
    catch {
...
export async function requestWorkspaceScreenshot() {
    return await chrome.runtime.sendMessage({ type: 'capture_screenshot' });
}
export async function requestWorkspaceNoteMode(tabId) {
    return await chrome.tabs.sendMessage(tabId, {
        type: 'kaboom_draw_mode_start',
        started_by: 'user'
    });
}
export async function toggleWorkspaceRecording(recordingActive) {
    if (recordingActive) {
