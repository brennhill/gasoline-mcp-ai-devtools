...
(() => {
  ...
  // extension/background/version-check.js
  function getExtensionVersion() {
    const manifest = chrome.runtime.getManifest();
    return manifest.version || "0.0.0";
  }

  // extension/background/server.js
  ...
  // extension/content/ui/chat-panel-sse.js
  function connectChatStream(serverUrl, conversationId2, onMessage, onHistory, onError) {
    ...
    function connect() {
      ...
      const url = `${serverUrl}/chat/stream?conversation_id=${encodeURIComponent(conversationId2)}`;
      fetch(url, {
        method: "GET",
        headers: {
          ...getRequestHeaders(),
  ...
  // extension/content/ui/chat-panel.js
  var PANEL_ID = "gasoline-chat-panel";
  var MESSAGES_ID = "gasoline-chat-messages";
  var INPUT_ID2 = "gasoline-chat-panel-input";
  var PANEL_WIDTH = "400px";
  var sseConnection = null;
  var conversationId = "";
  var isPanelRemoving = false;
  ...
  function toggleChatPanel(serverUrl) {
  ...
  function showChatPanel(serverUrl) {
    ...
    requestAnimationFrame(() => {
      ...
      const input = document.getElementById(INPUT_ID2);
      if (input)
        input.focus();
    });
    isPanelOpen = true;
    sseConnection = connectChatStream(serverUrl, conversationId, (msg) => renderMessage(msg), (msgs) => {
  ...
  function createInputArea(serverUrl) {
    ...
    hint.textContent = "Enter send \xB7 Shift+Enter newline \xB7 Esc close";
  ...
  function sendMessage(serverUrl) {
    const input = document.getElementById(INPUT_ID2);
    if (!input)
      return;
    const message = input.value.trim();
