...
;(() => {
    ...
    chrome.storage.local.get(storageKeys, (result) => {
      for (const setting of SYNC_SETTINGS) {
        ...
        if (setting.isMode) {
          ...
          window.postMessage(
            { type: 'GASOLINE_SETTING', setting: setting.messageType, enabled: value, _nonce: pageNonce },
            window.location.origin
          )
        }
      }
  ...
  function safeSendMessage(msg) {
    ...
    try {
      ...
      if (e instanceof Error && e.message?.includes('Extension context invalidated')) {
        contextValid = false
        console.warn(
          '[Gasoline] Please refresh this page. The Gasoline extension was reloaded and this page still has the old content script. A page refresh will reconnect capture automatically.'
        )
      }
    }
  }
  ...
  function initWindowMessageListener() {
    window.addEventListener('message', (event) => {
      ...
      if (responseHandler) {
        if (requestId !== void 0) responseHandler(requestId, result)
        return
      }
  ...
  async function handleStateCommand(params) {
    ...
    return promiseRaceWithCleanup(deferred.promise, 5e3, { error: 'State command timeout' }, () =>
      window.removeEventListener('message', responseHandler)
    )
  }
  ...
  function executeInMainWorld(params, sendResponse) {
    ...
    setTimeout(
      createRequestTimeoutCleanup(requestId, /* @__PURE__ */ new Map([[requestId, sendResponse]]), {
        success: false,
  ...
  function initRuntimeMessageListener() {
    chrome.storage.local.get(['actionToastsEnabled', 'subtitlesEnabled'], (result) => {
      if (result.actionToastsEnabled !== void 0) actionToastsEnabled = result.actionToastsEnabled
      if (result.subtitlesEnabled !== void 0) subtitlesEnabled = result.subtitlesEnabled
    })
    const syncHandlers = {
      GASOLINE_PING: () => {},
