...
(() => {
  ...
  function handleExecuteJs(params, sendResponse) {
    ...
    setTimeout(createRequestTimeoutCleanup(requestId, /* @__PURE__ */ new Map([[requestId, sendResponse]]), { success: false, error: "timeout", message: "Execute request timed out after 30s" }), 3e4);
    window.postMessage({
  ...
  function handleA11yQuery(params, sendResponse) {
    ...
    setTimeout(createRequestTimeoutCleanup(requestId, /* @__PURE__ */ new Map([[requestId, sendResponse]]), { error: "Accessibility audit timeout" }), 3e4);
    window.postMessage({
