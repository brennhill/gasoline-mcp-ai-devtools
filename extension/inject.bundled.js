...
function serializeObject(value, depth, seen) {
  ...
  seen.add(value)
  if (value.nodeType) return serializeDOMNode(value)
...
function truncateRequestBody(body) {
  if (body === null || body === void 0) return { body: null, truncated: false }
  ...
  window.postMessage(
    {
      ...
      payload: {
        ...
        message:
          payload.message ||
          payload.error ||
          (payload.args?.[0] !== null && payload.args?.[0] !== void 0 ? String(payload.args[0]) : ''),
...
function createConnectionTracker(id, url) {
  const tracker = {
    ...
    getMessageRate() {
      ...
      if (lastTime === void 0 || firstTime === void 0) return this._messageTimestamps.length
...
function collectAttributes(el) {
  if (!el.attributes || el.attributes.length === 0) return void 0
  const attrs = {}
...
function collectBoundingBox(el) {
  if (!el.getBoundingClientRect) return void 0
  const rect = el.getBoundingClientRect()
  return { x: rect.x, y: rect.y, width: rect.width, height: rect.height }
}
function collectStyles(el, includeStyles, styleProps) {
  if (!includeStyles || typeof window.getComputedStyle !== 'function') return void 0
...
function collectChildren(el, includeChildren, maxDepth, currentDepth) {
  if (!includeChildren || currentDepth >= maxDepth || !el.children || el.children.length === 0) return void 0
  const children = []
...
function safeSerializeForExecute(value, depth = 0, seen = /* @__PURE__ */ new WeakSet()) {
  ...
  if (type === 'object') return serializeObject2(value, depth, seen)
  return String(value)
}
    ...
    try {
      ...
      if (
        ...
        deferred.resolve({
          ...
          error: 'csp_blocked',
          message:
            'This page has a Content Security Policy that blocks script execution in the MAIN world. Use world: "isolated" to bypass CSP (DOM access only, no page JS globals). With world: "auto" (default), this fallback happens automatically.'
        })
      } else {
        deferred.resolve({
          success: false,
...
if (typeof window !== 'undefined') {
  window.addEventListener('message', (event) => {
    if (event.source !== window || event.origin !== window.location.origin) return
    if (pageNonce2 && event.data?._nonce !== pageNonce2) return
