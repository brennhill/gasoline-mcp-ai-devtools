...
class TestGenerateDefaultConfig(unittest.TestCase):
    ...
    def test_honors_binary_path_override(self):
        cfg = generate_default_config(binary_path="/tmp/kaboom-bin")
        self.assertEqual(cfg["mcpServers"][MCP_SERVER_NAME]["command"], "/tmp/kaboom-bin")
...
class TestBuildMcpEntry(unittest.TestCase):
    ...
    def test_honors_binary_path_override(self):
        entry = build_mcp_entry(binary_path="/tmp/kaboom-bin")
        parsed = json.loads(entry)
        self.assertEqual(parsed["command"], "/tmp/kaboom-bin")
...
class TestInstallToClient(unittest.TestCase):
    def test_creates_new_file_config(self):
        ...
        try:
            ...
            self.assertEqual(written["mcpServers"][MCP_SERVER_NAME]["command"], "/tmp/kaboom-bin")
        finally:
            shutil.rmtree(tmp)
    ...
    def test_removes_kaboom_and_strum_entries_before_writing(self):
        tmp = tempfile.mkdtemp()
        try:
            cfg_path = os.path.join(tmp, "mcp.json")
                ...
                json.dump(
                    {
                        "mcpServers": {
                            "other": {"command": "other"},
                            "kaboom": {"command": "kaboom-mcp"},
                            "kaboom-agentic-browser": {"command": "kaboom-agentic-browser"},
            ...
            with open(cfg_path, encoding="utf-8") as f:
                written = json.load(f)
            self.assertIn(MCP_SERVER_NAME, written["mcpServers"])
            self.assertNotIn("kaboom", written["mcpServers"])
            self.assertNotIn("kaboom-agentic-browser", written["mcpServers"])
...
class TestSkillCleanup(unittest.TestCase):
    def test_install_bundled_skills_removes_kaboom_and_strum_legacy_files(self):
        tmp = tempfile.mkdtemp()
        original_env = {key: os.environ.get(key) for key in ["KABOOM_CLAUDE_SKILLS_DIR", "KABOOM_SKILL_TARGETS"]}
        try:
            claude_root = os.path.join(tmp, "claude-skills")
            os.makedirs(claude_root, exist_ok=True)
            with open(os.path.join(claude_root, "kaboom-debug.md"), "w", encoding="utf-8") as f:
                f.write("<!-- kaboom-managed-skill id:debug version:1 -->\nold kaboom skill\n")
            with open(os.path.join(claude_root, "strum-debug.md"), "w", encoding="utf-8") as f:
                f.write("<!-- strum-managed-skill id:debug version:1 -->\nold strum skill\n")

            os.environ["KABOOM_CLAUDE_SKILLS_DIR"] = claude_root
            os.environ["KABOOM_SKILL_TARGETS"] = "claude"
            result = install_bundled_skills(verbose=False)

            self.assertFalse(result["skipped"])
            self.assertGreaterEqual(result["summary"]["legacy_removed"], 2)
            self.assertFalse(os.path.exists(os.path.join(claude_root, "kaboom-debug.md")))
