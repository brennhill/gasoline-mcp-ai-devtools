...
2. Run the install script:
   ...
   - Install the MCP server binary (`strum`) via npm. To avoid version mismatches,
     the script asks for your extension version (check `chrome://extensions` → Strum AI DevTools).
     Press Enter to install the latest version instead.

   > **Note:** Unlike the standard MCP setup (`npx strum` in `.mcp.json`), this installs
...
6. The Strum AI DevTools extension should now appear in your extensions list
