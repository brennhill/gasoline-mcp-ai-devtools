...
func TestParseSummaryParam(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "nil args defaults to true",
			args: nil,
			want: true,
		},
		{
			name: "empty args defaults to true",
			args: json.RawMessage(``),
			want: true,
		},
		{
			name: "empty object defaults to true",
			args: json.RawMessage(`{}`),
			want: true,
		},
		{
			name: "summary omitted defaults to true",
			args: json.RawMessage(`{"other":"field"}`),
			want: true,
		},
		{
			name: "summary explicitly true",
			args: json.RawMessage(`{"summary":true}`),
			want: true,
		},
		{
			name: "summary explicitly false",
	...
	_ = boolPtr // suppress unused if needed

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := parseSummaryParam(tc.args)
			if got != tc.want {
				t.Errorf("parseSummaryParam(%q) = %v, want %v", string(tc.args), got, tc.want)
			}
		})
	}
}
