# Kaboom Workflows — Claude Code Plugin

Workflow commands that compose Kaboom MCP's 5 browser telemetry tools into complete, opinionated workflows for debugging, auditing, recording, and interactive development.

## Prerequisites

- [Kaboom MCP](https://github.com/brennhill/Kaboom-Browser-AI-Devtools-MCP) installed and running
...
```bash
ln -s /path/to/kaboom/plugin/kaboom-workflows ~/.claude/plugins/kaboom-workflows
```

### Option 2: Copy

```bash
cp -r /path/to/kaboom/plugin/kaboom-workflows ~/.claude/plugins/kaboom-workflows
```
...
3. Translates commands to Kaboom tool calls
...
Automatically activates when any Kaboom tool call fails with a connection error. Runs health diagnostics, identifies whether the daemon, extension, or tab tracking is the issue, and guides you through recovery.
...
The commands compose Kaboom's 5 MCP tools (`observe`, `analyze`, `generate`, `interact`, `configure`) into multi-step workflows that would otherwise require knowing the tool API surface.

```
┌──────────────────────────────────────┐
│         Claude Code                  │
│  ┌────────────────────────────────┐  │
│  │  kaboom-workflows plugin     │  │
│  │  ┌──────┐ ┌──────┐ ┌────────┐ │  │
│  │  │debug │ │seo   │ │record  │ │  │
│  │  │-ui   │ │-audit│ │-wkflow │ │  │
│  │  └──┬───┘ └──┬───┘ └───┬────┘ │  │
