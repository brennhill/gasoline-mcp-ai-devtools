...
## v5.1: Blocking Requirements

**MANDATORY** completion before v5.1 release. Improves LLM token efficiency and reduces model confusion.

### Tool Surface Consolidation: 20 → 4 Tools

Collapse granular tool surface into 4 composite tools organized by intent: **read / create / configure / act**. Current 20-tool design causes LLM context bloat and decision confusion. Target: **80% reduction** in tool count.

**Decision rationale:** Tool selection is the expensive LLM decision (branching over N tools); parameter selection is cheap (enum pattern-matching). Four tools with unambiguous intent mapping eliminates tool selection confusion entirely. See [architecture.md](/.claude/docs/architecture.md) for the full decision record.
...
| Tool | Intent | Selector | Modes |
|------|--------|----------|-------|
| `observe` | "Show me something" | `what` | errors, logs, network, websocket_events, websocket_status, actions, vitals, page, tabs, pilot, performance, api, accessibility, changes, timeline, security_audit, third_party_audit, security_diff |
| `generate` | "Create something" | `type` | reproduction, test, pr_summary, sarif, har, csp, sri |
| `configure` | "Change a setting" | `action` | store, load, noise_rule, dismiss, clear, query_dom, diff_sessions, validate_api, audit_log, health, streaming |
| `interact` | "Control the browser" | `action` | highlight, save_state, load_state, list_states, delete_state, execute_js, navigate, refresh, back, forward, new_tab |

**What was absorbed:**
- `analyze` → merged into `observe` (both are "read" operations)
- `security` scanning tools → merged into `observe` (audit/scan = read)
- `security` generation tools → merged into `generate` (CSP/SRI = create)
- `configure_streaming` → merged into `configure` (streaming config = settings)
- All 4 AI Web Pilot tools → merged into `interact`
