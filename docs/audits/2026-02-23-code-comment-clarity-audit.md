...
- Updated source-header tooling to enforce `Purpose/Why/Docs` (not just `Purpose/Docs`) for `src/`, `cmd/`, and `internal/`.
...
- `node scripts/docs/check-source-headers.js` passes (`Purpose/Why/Docs` required).
