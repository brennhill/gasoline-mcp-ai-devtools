...
func (t *SSHTailer) Start(ctx context.Context) error {
	for {
		...
		if err != nil {
			t.recordError(err)
			fmt.Printf("[ssh-tailer] Connection failed: %v, retrying in 5s\n", err)
...
| Authentication failed | Fail permanently (user intervention needed) |
| Stream closed | Reconnect after 5 seconds |
| Permission denied | Fail (can't read log file) |
| File not found | Fail (log path wrong) |
| Network timeout | Retry after 5 seconds |
| SSH key not found | Fail at startup |
