...
| # | Issue | Severity | Location | Details |
|---|-------|----------|----------|---------|
| CR-21 | Guide says analyze is async, schema says sync-by-default | High | `cmd/dev-console/playbooks.go` guide text vs `tools_schema.go` | Guide and playbooks show async polling pattern ("analyze dispatches... poll with observe"). Schema description says "Synchronous Mode (Default)". LLMs following guide will unnecessarily poll. **NOTE:** Code is correct (sync-by-default). Docs need updating to match. |
| CR-22 | scope param has 3 different semantics | Medium | `tools_schema.go` observe/analyze/generate | `observe.scope` = "current_page"/"all" (data filter). `analyze.scope` = CSS selector. `generate.scope` = CSS selector. Same name, different types. **NOTE:** Need advice on best semantics before changing. |
...
| CR-28 | noise_rule has no inner schema for rules objects | Medium | `tools_schema.go` configure | `rules` array accepts objects but provides no JSON Schema for required fields. LLM must guess. **NOTE:** Need advice on what to do. |
| CR-29 | Quickstart missing examples for 3 of 5 tools | Medium | `cmd/dev-console/playbooks.go` | `gasoline://quickstart` has zero examples for `generate`, `configure`, or `interact`. **NOTE:** Defer until all other updates complete to avoid stale examples. |
...
| CR-33 | Playback engine is a stub — no browser replay | High | `internal/capture/playback.go` | `executeAction` returns `"ok"` for navigate/type/scroll without performing any browser action (commented "In real implementation, would navigate browser"). The `click` path checks selector validity but never sends CDP commands. No Chrome DevTools Protocol integration exists. **STATUS:** Known limitation — playback is a validation-only stub, not a functional replay engine. |
...
| CR-35 | Recording lost if server crashes before StopRecording | High | `internal/capture/recording.go` | Recording state (`Recording.Actions`) held in memory until `StopRecording` calls `persistRecordingToDisk`. No periodic flush, checkpoint, or write-ahead log. Server crash, OOM-kill, or SIGKILL loses all actions collected since `StartRecording`. **STATUS:** Known limitation — mitigation would require periodic flush to temp file during recording. |
| CR-36 | Service worker termination drops in-flight state | High | Extension service worker | Chrome kills service workers after 30s of inactivity. All module-level state is reset: `syncClient`, `connectionStatus`, `extensionLogQueue`, `recordingState`, `_captureOverrides`, `aiControlled`, error deduplication state. `wasServiceWorkerRestarted` detection exists (`storage-utils.ts`) but only restores user preferences from `chrome.storage.local` — no runtime state recovery. **STATUS:** Known limitation — recovery would require saving transient state to `chrome.storage.session` before suspension. |
