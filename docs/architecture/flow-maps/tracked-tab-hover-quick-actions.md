...
Inject a floating quick-action launcher on tracked tabs so users can start annotation draw mode, start or stop recording, and take screenshots without reopening the popup.
...
3. `Draw` action dynamically loads `content/draw-mode.js` and calls `activateDrawMode('user')`.
4. `Rec` or `Stop` action sends `record_start` or `record_stop` to background recording listeners.
5. `Shot` action sends `captureScreenshot` to background message handlers.
6. Record button state stays aligned with `chrome.storage.local[gasoline_recording]` via initial read plus `chrome.storage.onChanged`.
