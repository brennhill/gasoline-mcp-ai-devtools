...
func (m *SecurityDiffManager) TakeSnapshot(name string, bodies []NetworkBody) (*SecuritySnapshot, error) {
	// Validate name
	if name == "" {
...
func (m *SecurityDiffManager) Compare(fromName, toName string, currentBodies []NetworkBody) (*SecurityDiffResult, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *SecurityDiffManager) HandleDiffSecurity(params json.RawMessage, bodies []NetworkBody) (interface{}, error) {
...
func (m *SecurityDiffManager) buildEphemeralSnapshot(bodies []NetworkBody) (*SecuritySnapshot, error) {
	snap := &SecuritySnapshot{
		Name:      "current",
		TakenAt:   time.Now(),
		Headers:   make(map[string]map[string]string),
