...
// builtinRules returns the set of always-active built-in noise rules
func builtinRules() []NoiseRule {
	...
	return []NoiseRule{
		...
		},
		{
			...
			Classification: "framework",
			MatchSpec: NoiseMatchSpec{
				URLRegex: `(__vite_ping|hot-update|__webpack_hmr|_next/webpack-hmr|webpack-dev-server)`,
			},
			CreatedAt: now,
		},
		{
		...
		},
		{
			...
			MatchSpec: NoiseMatchSpec{
				URLRegex: `(google-analytics\.com|segment\.io|mxpnl\.com|hotjar\.com|amplitude\.com|plausible\.io|posthog\.com)`,
			},
			CreatedAt: now,
		},
		{
...
func (nc *NoiseConfig) AddRules(rules []NoiseRule) error {
	...
	for _, r := range rules {
		...
		nc.userIDCounter++
		r.ID = fmt.Sprintf("user_%d", nc.userIDCounter)
		r.CreatedAt = time.Now()
		nc.rules = append(nc.rules, r)
	}
	nc.recompile()
	return nil
}
...
func (nc *NoiseConfig) isConsoleCoveredLocked(msg string, entries []LogEntry) bool {
	for _, cr := range nc.compiled {
		if cr.rule.Category != "console" {
			continue
		}
		if cr.messageRegex != nil && cr.messageRegex.MatchString(msg) {
			return true
		}
	}
...
func (nc *NoiseConfig) isURLCoveredLocked(path string) bool {
	for _, cr := range nc.compiled {
		...
		if cr.urlRegex != nil && cr.urlRegex.MatchString(path) {
			return true
		}
	}
	return false
