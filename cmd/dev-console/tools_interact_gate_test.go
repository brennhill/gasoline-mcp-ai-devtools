...
//
// Run: go test ./cmd/dev-console -run "TestRequireExtension|TestRequireCSP|TestGateOrder|TestDiagnosticHint|TestNavigate_Ext|TestExecuteJS_CSP|TestClick_Ext|TestSubtitle_No" -v
package main
...
func TestRequireCSPClear_ScriptExec(t *testing.T) {
	t.Parallel()
	env := newGateTestEnv(t)
	env.capture.SetCSPStatusForTest(true, "script_exec")

	req := JSONRPCRequest{JSONRPC: "2.0", ID: json.RawMessage(`1`)}
	resp, blocked := env.handler.requireCSPClear(req)
	if !blocked {
		t.Fatal("expected requireCSPClear to block when CSP restricts script_exec")
	}
	code := extractErrorCode(t, resp)
	if code != ErrExtError {
...
func TestRequireCSPClear_PageBlocked(t *testing.T) {
	...
	req := JSONRPCRequest{JSONRPC: "2.0", ID: json.RawMessage(`1`)}
	_, blocked := env.handler.requireCSPClear(req)
	if !blocked {
		t.Fatal("expected requireCSPClear to block when CSP level is page_blocked")
	}
}
...
func TestExecuteJS_CSP_FastFail(t *testing.T) {
	t.Parallel()
	env := newGateTestEnv(t)
	env.enablePilot(t)
	env.simulateConnection(t)
	...
	code := extractErrorCode(t, resp)
	if code != ErrExtError {
		t.Fatalf("expected %q error for execute_js with CSP restricted, got %q", ErrExtError, code)
	}
}
