...
func (s *AnnotationStore) WaitForSession(timeout time.Duration) (*AnnotationSession, bool) {
	...
	// Check if a qualifying session already exists
	if session := s.getSessionSince(sinceTs); session != nil {
		return session, false
	}

	// Block until new session arrives, timeout, or store closes
	timer := time.NewTimer(timeout)
	defer timer.Stop()

	select {
	case <-ch:
		session := s.getSessionSince(sinceTs)
		return session, session == nil
	case <-timer.C:
		// One last check — session may have arrived just before timeout
		if session := s.getSessionSince(sinceTs); session != nil {
			return session, false
		}
		return nil, true
...
// GetNamedSession returns a named multi-page session if not expired.
...
func (s *AnnotationStore) WaitForNamedSession(name string, timeout time.Duration) (*NamedAnnotationSession, bool) {
	...
	// Check if the named session has been updated since draw started
