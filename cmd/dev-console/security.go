...
// NetworkBody extension fields — these are added to the existing NetworkBody
// via a wrapper since we can't modify types.go without broader impact.
// The test uses the extended fields directly on NetworkBody.
...
func (s *SecurityScanner) checkCredentials(bodies []NetworkBody, entries []LogEntry) []SecurityFinding {
	var findings []SecurityFinding
...
func (s *SecurityScanner) scanURLForCredentials(body NetworkBody) []SecurityFinding {
	var findings []SecurityFinding
...
func (s *SecurityScanner) checkSecurityHeaders(bodies []NetworkBody) []SecurityFinding {
	var findings []SecurityFinding
...
func (s *SecurityScanner) checkCookies(bodies []NetworkBody) []SecurityFinding {
	var findings []SecurityFinding
...
func (s *SecurityScanner) checkTransport(bodies []NetworkBody, pageURLs []string) []SecurityFinding {
	var findings []SecurityFinding
...
func (s *SecurityScanner) checkAuthPatterns(bodies []NetworkBody) []SecurityFinding {
	var findings []SecurityFinding
...
func isHTMLResponse(body NetworkBody) bool {
...
func filterBodiesByURL(bodies []NetworkBody, filter string) []NetworkBody {
	if filter == "" {
		return bodies
	}
...
func buildSummary(findings []SecurityFinding, bodies []NetworkBody) ScanSummary {
