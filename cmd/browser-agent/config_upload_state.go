...
// Always defaults to ~/kaboom-upload-dir when no --upload-dir is specified.
...
func initUploadSecurity(enabled bool, dir string, denyPatterns multiFlag) {
	if dir == "" {
		...
		if err != nil {
			if enabled {
				fmt.Fprintf(os.Stderr, "[Kaboom] Cannot determine home directory for default upload dir: %v\n", err)
				os.Exit(1)
			}
		...
		if err := os.MkdirAll(dir, 0o755); err != nil {
			if enabled {
				fmt.Fprintf(os.Stderr, "[Kaboom] Cannot create default upload dir %s: %v\n", dir, err)
				os.Exit(1)
			}
			uploadSecurityConfig = &UploadSecurity{}
			return
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[Kaboom] Upload security validation failed: %v\n", err)
		os.Exit(1)
	}
...
func validatePort(port int) {
	if port < 1 || port > 65535 {
		fmt.Fprintf(os.Stderr, "[Kaboom] Invalid port: %d (must be 1-65535)\n", port)
		os.Exit(1)
	}
}
