...
test('uninstall.executeUninstall is idempotent in dryRun mode', () => {
  ...
  assert.strictEqual(
    result1.removed.length,
    result2.removed.length,
    'dryRun results should be consistent'
  );
  assert.strictEqual(
    result1.notConfigured.length,
    result2.notConfigured.length,
    'dryRun results should be consistent'
  );
});
...
test('uninstall.executeUninstall checks all 4 config locations', () => {
  ...
  const allTools = result.removed.map(r => r.name)
