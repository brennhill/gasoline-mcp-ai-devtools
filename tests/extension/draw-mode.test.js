...
function createMockElement(tag = 'div') {
  const el = {
    ...
    id: '',
    className: '',
    classList: { _items: [], add(c) { this._items.push(c) }, contains(c) { return this._items.includes(c) }, [Symbol.iterator]() { return this._items[Symbol.iterator]() } },
    style: {},
    dataset: {},
    textContent: '',
    children: [],
...
function setupGlobals() {
  ...
  globalThis.requestAnimationFrame = mock.fn((cb) => { cb(); return 1 })
...
describe('Draw Mode — Activation/Deactivation', () => {
  ...
  test('overlay is removed on deactivation', () => {
    dm.activateDrawMode('user')
    const overlayBefore = documentBody.children.length
    dm.deactivateDrawMode()
...
describe('Draw Mode — Export', () => {
  ...
  test('composites annotations onto screenshot', async () => {
    const dataUrl = 'data:image/png;base64,abc'
    const annotations = [
      { rect: { x: 100, y: 200, width: 150, height: 50 }, text: 'make darker', id: 'ann_1' },
    ]
...
describe('Draw Mode — Persistence', () => {
  ...
  test('clearAnnotations triggers persistence', (t) => {
    dm.activateDrawMode('user')
...
describe('Draw Mode — State Leak Prevention', () => {
  ...
  test('deactivateDrawMode clears annotations for next session', () => {
    ...
    // First session
    const result1 = dm.deactivateDrawMode()
