...
function createHandlerFunctions() {
  const debugLog = (level, msg, data) => { debugLogCalls.push({ level, msg, data }) }
  ...
  async function handleDrawModeCompleted(message, sender, syncClient) {
    const tabId = sender.tab?.id
    if (!tabId) return
  ...
  function installDrawModeCommandListener() {
    ...
    chrome.commands.onCommand.addListener(async (command) => {
      ...
      try {
        ...
        if (!tab?.id) return
        try {
          const result = await chrome.tabs.sendMessage(tab.id, { type: 'GASOLINE_GET_ANNOTATIONS' })
          if (result?.draw_mode_active) {
            await chrome.tabs.sendMessage(tab.id, { type: 'GASOLINE_DRAW_MODE_STOP' })
          } else {
            await chrome.tabs.sendMessage(tab.id, { type: 'GASOLINE_DRAW_MODE_START', started_by: 'user' })
          }
        } catch {
          try {
            await chrome.tabs.sendMessage(tab.id, { type: 'GASOLINE_DRAW_MODE_START', started_by: 'user' })
...
describe('Draw Mode Handler — handleDrawModeQuery', () => {
  ...
  test('start action sends message to content script and returns result', async () => {
    ...
    await handler.handleDrawModeQuery(
      { id: 'q1', params: '{"action":"start"}' },
      42,
      sendResult, null, 'sync-client'
    )
  ...
  test('unknown action returns error', async () => {
    ...
    await handler.handleDrawModeQuery(
      { id: 'q4', params: { action: 'stop' } },
      42, sendResult, null, 'c'
    )
