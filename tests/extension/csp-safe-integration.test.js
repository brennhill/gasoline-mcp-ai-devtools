...
describe('CSP-safe integration: fallback chain', () => {
  ...
  test('CSP error from content script falls back to ISOLATED world', async () => {
    globalThis.chrome = {
      tabs: {
        sendMessage: mock.fn(() =>
      ...
      scripting: {
        executeScript: mock.fn(() =>
          Promise.resolve([{ result: { success: true, result: 'isolated_works' } }])
        )
      }
    }
    ...
    assert.strictEqual(result.success, true)
    assert.strictEqual(result.result, 'isolated_works')
    assert.strictEqual(result.execution_mode, 'isolated_csp_fallback')
    // scripting API should have been called with ISOLATED world
  ...
  test('world=isolated routes directly to ISOLATED world', async () => {
    globalThis.chrome = {
      tabs: { sendMessage: mock.fn() },
      scripting: {
        executeScript: mock.fn(() =>
          Promise.resolve([{ result: { success: true, result: 'isolated' } }])
        )
      }
    }
    ...
    // Should call scripting API with ISOLATED world
    const scriptCall = globalThis.chrome.scripting.executeScript.mock.calls[0]
    assert.strictEqual(scriptCall.arguments[0].world, 'ISOLATED')
  })

  test('world=isolated falls back to structured executor on CSP failure', async () => {
    let callCount = 0
    globalThis.chrome = {
      tabs: { sendMessage: mock.fn() },
      scripting: {
        executeScript: mock.fn((...args) => {
          callCount++
          if (callCount === 1) {
            // ISOLATED world fails due to CSP
          ...
          return Promise.resolve([{
            result: { success: true, result: 'structured_fallback', execution_mode: 'csp_safe_structured' }
          }])
        })
      }
    }
  ...
  test('CSP error with ISOLATED failure falls back to structured executor', async () => {
    let callCount = 0
    globalThis.chrome = {
      tabs: {
        sendMessage: mock.fn(() =>
